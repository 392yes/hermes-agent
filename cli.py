    ...
    def _show_status(self):
        ...
        try:
            ...
            if read_mode(get_hermes_home()).get("mode") == MODE_CLARA_LEAD:
                model_short = "claude-code"
                provider_display = "claude-code-cli"
        except Exception:
            provider_display = self.provider
            ...
            def run_agent():
                ...
                try:
                    ...
                    if route_via_claude_code:
                        ...
                        _bridge_progress_callback(
                            "bridge.started",
                            "Claude Agent SDK 런타임을 시작했습니다. SDK 중간 응답은 Hermes 기본 박스에 표시합니다.",
                            {},
                        )
                ...
                try:
                    ...
                    if _file_drop:
                        ...
                        if _file_drop["is_image"]:
                            # Mouse-dragging or pasting an image file into VS Code's
                            # integrated terminal should behave like the terminal used
                            # to behave for Sangkun: insert/send the file path as plain
                            # text.  Do NOT auto-attach images from dropped paths here;
                            # explicit image attachment remains available via /image,
                            # /paste, and Alt+V.
                            user_input = str(_drop_path) + (f"\n{_remainder}" if _remainder else "")
                            _file_drop = None
                        else:
